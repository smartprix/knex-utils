...
/**
 * @typedef {object} knexFileConf
 * @property {string} client
 * @property {boolean} debug
 * @property {connectionConf} connection
...
/**
 * @returns {Object.<string, knexFileConf>}
 */
...
function getKnex() {
	if (!globalKnex) {
		const knexFileConf = getKnexFile();
		const env = process.env.NODE_ENV || 'development';
		const dbConfig = knexFileConf[env];
...
/**
 * @param {Partial<Console>} loggerInstance
 */
...
async function seedFolder(folderPath) {
	...
	return new Promise((resolve, reject) => {
		fs.readdir(folderPath, (err, tables) => {
			...
			Promise.all(tables.map(tableData => knex(tableData.name).then(() => {
...
async function copyDb(knex, oldDbName, newDbName) {
	...
	const tempKnex = Knex(_setDb(_reducePoolToOne(dbConfig), 'postgres'));
	logger.time(`Copied DB ${oldDbName} to ${newDbName}`);
	...
	await tempKnex.destroy();
	logger.timeEnd(`Copied DB ${oldDbName} to ${newDbName}`);
...
async function refreshDb(env) {
	...
	logger.time('Ran migrations');
	await knex.migrate.latest();
	logger.timeEnd('Ran migrations');

	logger.time('Seeded data');
	await knex.seed.run();
	logger.timeEnd('Seeded data');
